/***********************************************************************************************
 Program:      Quarterly Security Paper Report Distribution
 Author:       Vanessa Kohl
 Date:         September 2024
 Purpose:      This program generates and sends quarterly email reports to county offices in
               Colorado, summarizing security paper issuance and discrepancies for vital records.
               Reports are categorized based on whether or not the office has discrepancies in
               Document Control Number (DCN) logging, which helps ensure accountability and
               prevent fraud.

 Libraries:
    - `WrkBk`: Reads data from an Excel workbook containing quarterly security paper data.
    - `Work`: Processes data for email distribution.

 Input Data:
    - Excel file: Quarterly log of security paper issuance and discrepancies by county.

 Output:
    - Emails to county offices summarizing their issuance and discrepancy statuses.
    - Categorized reports for offices with and without discrepancies.

 Process Overview:
    1. **Data Import**: Reads and processes the quarterly data from an Excel file, filtering
       and transforming relevant columns.
    2. **Data Preparation**: Prepares email addresses, county names, and summarizes discrepancies
       for each county. Adjusts formats for clear report presentation.
    3. **Distribution of Clean Reports**: Generates and sends emails to counties with no
       discrepancies in their security paper issuance.
    4. **Distribution of Discrepancy Reports**: Generates and sends emails to counties with
       identified discrepancies, including specific DCN issues.
    5. **Symbolic Variables**: Uses macros to loop through counties and insert dynamically
       generated values in each email.

 Modifications:
    - [Date: November 04 2024] - Recovered program created.

 Notes:
    - Ensure the input Excel file path is updated for each quarterly report.
    - Customize email content for each quarter by adjusting the subject, body text, and reporting
      dates as needed.
    - Discrepancies are presented directly in the email content to provide immediate feedback
      to each county office.
...
DATA _NULL_;
        ...
        PUT @1 "dates or the range of security paper document control numbers you wish to check. Click generate to view the report. Any gaps, errors or duplications will ";
        PUT @1 "highlight in red. ";
		PUT @1 " ";
		PUT @1 "Discrepancies:";
